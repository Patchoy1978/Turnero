...
def mostrar_informacion_turno(funcion):
    ...
    locale.setlocale(locale.LC_TIME, 'es_CO.utf8') # con esto damos formato a la fecha y hora qu obtenemos del sistema
    ...
    def interna(self, *args, **kwargs):
        ...
        resultado = funcion(self, *args, **kwargs) # almacenamos el resultado de una funcion en una variable
        print("\033[1m",datetime.now().strftime('%A, %d de %B de %Y').center(38),"\033[0m") # obtenemos la fecha actual
        print("\033[1m",datetime.now().strftime('%H:%M:%S').center(38),"\033[0m\n") # obtenemos la hora actual
        print(f'\033[1m{'Pronto será Atendido!'.center(42)}\033[0m\n\033[1m') # mensaje de anuncio
        return resultado # retornamos la funcnion que se le pasa

    return interna # retornamos la funcion interna
...
def categorias():
    ...
    # se almacena en una variable la lista de las categorias
    ...
    while True:
        ...
        try:
            ...
            # el condicional para las opciones
            if opcion == '1':
